...
interface Event {
  ...
  ticketTypes: Array<{
    name: string;
    price: number | string; // Prisma returns Decimal as string/number depending on config, usually Decimal.toJson is string? Prisma client returns Decimal object. We should be careful.
    // In Server Components passing to Client Components, dates and Decimals need to be serialized.
    quantity: number;
    sold: number;
    isActive: boolean;
  }>;
...
interface EventListClientProps {
  initialEvents: any[]; // Using any to bypass complex serialization types for now, will cast inside
}

export function EventListClient({ initialEvents }: EventListClientProps) {
  const [activeTab, setActiveTab] = useState<"upcoming" | "past">("upcoming");

  // Parse dates back if they were serialized
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {displayEvents.length > 0 ? (
          displayEvents.map((event, index) => (
            ...
            >
              <Card className="overflow-hidden hover:shadow-xl transition-all group border-0 shadow-md h-full flex flex-col">
                {/* Cover Image */}
                <div
                  className={`relative aspect-video overflow-hidden bg-gradient-to-br ${event.coverImageUrl ? "" : getEventGradient(event.id)}`}
                >
                  {event.coverImageUrl ? (
                    <img
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="space-y-1">
                      {event.ticketTypes
                        ...
                        .slice(0, 3)
                        .map((ticketType: any, index: number) => (
                          <div
                            key={index}
                      ...
                      {event.ticketTypes.filter((tt: any) => tt.isActive)
                        .length > 3 && (
