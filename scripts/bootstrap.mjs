...
// Check if Auth0 CLI is authenticated, if not, try to login
const authCheck = ora({
  text: `Checking Auth0 CLI authentication`,
}).start()
try {
  await $`auth0 tenants list`
  authCheck.succeed()
} catch (e) {
  authCheck.text = `Auth0 CLI not authenticated, attempting login...`
  try {
    // Try to login non-interactively
    await $`auth0 login --scopes "update:tenant_settings,create:connections,create:client_grants,create:email_templates,update:guardian_factors" --no-input`
    authCheck.succeed("Auth0 CLI authenticated successfully")
  } catch (loginError) {
    authCheck.fail(
      "Auth0 CLI authentication failed. Please run 'auth0 login' manually first."
    )
    console.log("\nTo fix this, run:")
    console.log("  auth0 login --scopes \"update:tenant_settings,create:connections,create:client_grants,create:email_templates,update:guardian_factors\"")
    process.exit(1)
  }
}

// Use the tenant domain from the login session
...
try {
  ...
  const createClientArgs = [
    ...
    "--description", "The EventApp client to manage tenant resources and facilitate account creation.",
...
try {
  ...
  const createRoleArgs = [
    "roles", "create",
    "--name", "admin",
    "--description", "Manage the organization's events and ticket purchases.",
    "--json", "--no-input"
  ];
